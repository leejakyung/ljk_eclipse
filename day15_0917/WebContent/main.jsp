...
<script type="text/javascript">
	...
	function del(mid) { 
		...
		if (result == true) {
			/* document.location.href = "deletemsg.do?mid="+mid+"&mcnt="+mcnt+"&selUser="+selUser; // selUser undefind 에러*/ 
			document.location.href = "deletemsg.do?mid="+mid+"&mcnt=${mcnt}&selUser=${selUser}"; 
		} else {
			return;
		}
	}
	function rdel(rid, mid) { 
...
<body>
	...
	<div class="content">
		<ol>
			<li><a href="main.do"><img alt="로고" src="image/pig.png"
		...
		<c:forEach var="v" items="${datas}">
			...
			<h3>
				[${m.userID}] ${m.msg} &gt;&gt; [좋아요 ${m.favcount} | 댓글
				${m.replycount} | ${m.mdate}] <a
					href="fvmsg.do?mid=${m.mid}&mcnt=${mcnt}&selUser=${selUser}">♥</a>

				<c:if test="${seUser == m.userID}">
					<input type="button" value="삭제" onclick="del(${m.mid})">
					<!-- 스크립트에 받아온 인자를 넣어줌  -->
			...
			<c:if test="${seUser != null}">
				<form method="post" action="newrp.do">
					...
					<input type="hidden" name="mid" value="${m.mid}"> <input
						type="hidden" name="userID" value="${seUser}"> <input
						type="hidden" name="selUser" value="${selUser}"> <input
			...
			<ol>
				<c:forEach var="r" items="${v.rlist}">
					<li>${r.userID}>> ${r.rmsg} [${r.rdate}] <c:if
							test="${seUser == r.userID}">
							<input type="button" value="-" onclick="rdel(${r.rid}, ${r.mid})">
						<%-- 	<a
								href="deleterp.do?rid=${r.rid}&mid=${r.mid}&mcnt=${mcnt}&selUser=${selUser}">삭제</a> --%>
						</c:if>
					</li>
				</c:forEach>
			</ol>
		...
		<ol>
			<c:forEach var="u" items="${newUsers}">
				<li><a href="main.do?mcnt=${mcnt}&selUser=${u.userID}">${u.name}</a>님
					가입</li>
			</c:forEach>
		</ol>
	</div>
	...
	<div class="img">
		<ul>
			<li><a href="https://www.youtube.com/watch?v=xlmPbjAcRXQ"><img
					alt="뽀로로이미지" src="image/pororo.png"></a></li>
			<li><a
				href="https://www.youtube.com/watch?v=pAkE0mK1_ds&list=PLXO7jFcjiAWC6p2c6pd6L98eBqMKJS4zV"><img
					alt="디즈니이미지" src="image/dog.jpg"></a></li>
			<li><a href="https://www.youtube.com/watch?v=BmDT1eq23l4"><img
					alt="버섯이미지" src="image/mushroom.ico"></a></li>
		</ul>
	</div>
