...
<%
	...
	if(action.equals("main")){
		...
		response.sendRedirect("control.jsp?action=main"); // 로그아웃시 메인페이지로 이동
		...
		out.println(memberDAO.checkID((String)session.getAttribute(memberVO.getUserID()))); // 입력받은 아이디 존재 여부 확인
		...
		if (request.getParameter("search") != null) {
			...
			//->search의 경우 form에 있는 데이터를 가져오는것임으로 request.get밖에 사용하지 못함
		...
		ArrayList<MessageVO> datas = null;
		datas = messageDAO.getMyDBList(memberVO.getUserID());
		session.setAttribute("datas", datas);
		System.out.println("내 글 보기!");
		pageContext.forward("list.jsp");
	} else if (action.equals("insert")) { // 게시글 등록
		if (messageDAO.insertDB(messageVO, memberVO)) {
			response.sendRedirect("control.jsp?action=list");
		} else {
		...
		MemberVO memData = memberDAO.getMemData(memberVO);
		System.out.println(memData);
		session.setAttribute("memData", memData);
		pageContext.forward("mypage.jsp");
	} else if (action.equals("deleteMem")){ // 정보 삭제시 메인페이지로 ...
		System.out.println("회원정보삭제");
		if (memberDAO.deleteMemDB(memberVO)) {
			response.sendRedirect("control.jsp?action=logout"); // 정보를 초기화 해야하기 때문에 logout으로 보내야함!
		} else {
			throw new Exception("MemberDB delete 오류 발생!");
		}
	} else if (action.equals("updateMem")){
		System.out.println("회원정보변경");
		if (memberDAO.updateMemDB(memberVO)) {
			response.sendRedirect("control.jsp?action=list"); // 정보 수정시 로그인페이지로
		} else {
			throw new Exception("MemberDB update 오류 발생!");
		}
